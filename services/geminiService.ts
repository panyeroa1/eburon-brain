...
const factCheckWithSearch = async (description: string): Promise<{ text: string; sources: { uri: string; title: string; }[] }> => {
  const prompt = `You are a fact-checker. Based on the following description of a social media video/post, search the web. Your primary goal is to find the original source or the earliest appearance of this content. Also, find any relevant context, news, or discussions about its authenticity. Description: '${description}'. Summarize your findings concisely.`;
...
const inferMetadata = async (description: string, imageFile: File | null): Promise<string> => {
    const prompt = `You are a digital forensics expert specializing in social media content. Based on the provided description and optional screenshot, analyze and infer potential metadata.
    ...
    **JSON Structure:**
    {
      ...
      "confidence": number (from 0.0 to 1.0, e.g., 0.95),
...
export const orchestrateAnalysis = async (
  ...
  let factCheckResult;
  try {
    factCheckResult = await factCheckWithSearch(description);
  ...
  try {
    metadataInferenceResult = await inferMetadata(description, imageFile);
  ...
  try {
    const finalResult = await synthesizeResults(description, visualAnalysisResult, factCheckResult, metadataInferenceResult, speechAnalysisResult);
